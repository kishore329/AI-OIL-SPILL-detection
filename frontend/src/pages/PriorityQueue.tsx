...
export default function PriorityQueue() {
  ...
  const getUrgencyBadge = (urgency: string) => {
    switch (urgency) {
      case "IMMEDIATE":
        return "bg-[#FFF1F2] text-[#C6283D] border-[#F5B5BC] font-extrabold";
      case "HIGH":
        return "bg-[#FFF8E8] text-[#A86A00] border-[#F3D58A] font-bold";
      case "ELEVATED":
        return "bg-[#EAF6FF] text-[#1268B3] border-[#A9D9F5] font-bold";
      default:
        return "bg-[#EAF8F4] text-[#087F68] border-[#9ADBC8]";
    }
  };
  ...
  const getRankMedal = (rank: number) => {
    if (rank === 1) return "bg-[#FFF1F2] text-[#C6283D] border-2 border-[#F5B5BC]";
    if (rank === 2) return "bg-[#FFF8E8] text-[#A86A00] border-2 border-[#F3D58A]";
    if (rank === 3) return "bg-[#EAF6FF] text-[#1268B3] border border-[#A9D9F5]";
    return "bg-[#F3FAFE] text-[#5E7183] border border-[#D9E8F2]";
  };
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[#D9E8F2] pb-5">
        <div>
          <div className="flex items-center gap-2.5">
            <div className="w-10 h-10 rounded-xl bg-[#FFF1F2] border border-[#F5B5BC] flex items-center justify-center text-[#C6283D]">
            ...
            <div>
              <h1 className="text-xl font-bold text-[#17324D] tracking-tight flex items-center gap-2">
                ...
                <span className="text-[10px] uppercase font-mono px-2 py-0.5 rounded font-bold bg-[#FFF1F2] text-[#C6283D] border border-[#F5B5BC]">Module 5</span>
              </h1>
              <p className="text-xs text-[#5E7183]">
                Deterministic algorithmic ranking answering: <span className="text-[#0B3A66] font-semibold italic">“Which incident should response teams handle first?”</span>
              </p>
            </div>
          </div>
        </div>
      ...
      {error && (
        <div className="p-3 bg-[#FFF1F2] border border-[#F5B5BC] rounded-xl text-[#C6283D] text-xs flex items-center justify-between font-medium">
          <span>{error}</span>
          <button onClick={fetchPriorityQueue} className="underline hover:font-bold">Retry</button>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-4 rounded-xl border border-[#F5B5BC] shadow-sm space-y-1 hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between">
            <span className="text-[#C6283D] text-xs font-semibold">Immediate Urgency</span>
            <AlertOctagon className="w-4 h-4 text-[#C6283D]" />
          </div>
          <div className="text-2xl font-black text-[#C6283D] font-mono">
            {loading ? "..." : immediateCount}
          </div>
          <div className="text-[10px] text-[#5E7183]">Requires instant boom / skimmer dispatch</div>
        </div>

        <div className="bg-white p-4 rounded-xl border border-[#F3D58A] shadow-sm space-y-1 hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between">
            <span className="text-[#A86A00] text-xs font-semibold">High Priority</span>
            <Flame className="w-4 h-4 text-[#A86A00]" />
          </div>
          <div className="text-2xl font-black text-[#A86A00] font-mono">
            {loading ? "..." : highCount}
          </div>
          <div className="text-[10px] text-[#5E7183]">Score range 60.0 – 79.9</div>
        </div>

        <div className="bg-white p-4 rounded-xl border border-[#D9E8F2] shadow-sm space-y-1 hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between">
            <span className="text-[#5E7183] text-xs font-medium">#1 Primary Target</span>
            <Shield className="w-4 h-4 text-[#1268B3]" />
          </div>
          <div className="text-lg font-black text-[#0B3A66] font-mono truncate">
            {loading ? "..." : queueData?.highest_priority_incident ?? "N/A"}
          </div>
          <div className="text-[10px] text-[#8A9AA8]">Top prioritized containment case</div>
        </div>

        <div className="bg-white p-4 rounded-xl border border-[#D9E8F2] shadow-sm space-y-1 hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between">
            <span className="text-[#5E7183] text-xs font-medium">Active Queue Size</span>
          ...
          <div className="text-[10px] text-[#8A9AA8]">Active non-resolved slicks</div>
        </div>
      </div>

      {/* Filter Bar */}
      <div className="bg-white p-3.5 rounded-xl border border-[#D9E8F2] shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div className="flex items-center gap-3 flex-1">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-1.5 bg-[#F8FCFF] border border-[#D9E8F2] rounded-lg text-xs text-[#17324D] placeholder-[#8A9AA8] focus:outline-none focus:border-[#1268B3]"
            />
          </div>

          <div className="flex items-center gap-1.5">
            <Filter className="w-3.5 h-3.5 text-[#5E7183]" />
            <select
              value={urgencyFilter}
              onChange={(e) => setUrgencyFilter(e.target.value)}
              className="py-1.5 px-2.5 bg-[#F8FCFF] border border-[#D9E8F2] rounded-lg text-xs text-[#17324D] focus:outline-none focus:border-[#1268B3] font-medium"
            >
        ...
        <div className="flex items-center gap-2 text-xs">
          <label className="flex items-center gap-2 cursor-pointer text-[#5E7183] select-none font-medium">
            <input
              type="checkbox"
              checked={includeResolved}
              onChange={(e) => setIncludeResolved(e.target.checked)}
              className="rounded accent-[#1268B3]"
            />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 space-y-3">
          {loading ? (
            <div className="bg-white rounded-xl border border-[#D9E8F2] py-16 flex flex-col items-center justify-center text-[#5E7183] text-xs">
            ...
            <div className="bg-white rounded-xl border border-[#D9E8F2] p-12 text-center text-xs text-[#5E7183]">
              ...
              return (
                <div
                  ...
                  className={`p-4 rounded-xl border transition-all duration-150 relative cursor-pointer ${
                    isSelected
                      ? "border-2 border-[#1268B3] bg-[#F3FAFE] shadow-md -translate-y-0.5"
                      : "bg-white border-[#D9E8F2] hover:border-[#1268B3]/50 hover:shadow-xs"
                  }`}
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex items-start gap-3">
                      <div
                        className={`w-9 h-9 rounded-xl border flex items-center justify-center font-extrabold text-sm font-mono shrink-0 shadow-xs ${getRankMedal(
                          item.rank
                        )}`}
                      >
                        #{item.rank}
                      ...
                      <div>
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <span
                            className={`text-[10px] font-bold px-1.5 py-0.5 rounded border ${getUrgencyBadge(
                              item.urgency_level
                            )}`}
                          >
                            {item.urgency_level}
                        ...
                        {/* Explainable Summary */}
                        <p className="text-xs text-[#5E7183] mt-1.5 line-clamp-2 leading-relaxed">
                          {item.reason}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="text-right shrink-0">
                      <div className="flex items-baseline justify-end gap-1">
                        <span className="text-xl font-extrabold font-mono text-[#0B3A66]">
                          {item.priority_score.toFixed(1)}
                        </span>
                        <span className="text-[10px] text-[#8A9AA8]">/ 100</span>
                      </div>
                      <div className="text-[10px] text-[#5E7183]">
                        Risk: <span className="font-mono text-[#17324D] font-bold">{item.risk_score.toFixed(1)}</span>
                      </div>
                    </div>
                  </div>

                  {/* Quantitative Chips Bar */}
                  <div className="flex items-center justify-between mt-3 pt-2.5 border-t border-[#EAF3F8] text-[11px] text-[#5E7183]">
                    <div className="flex items-center gap-3">
                      <span className="flex items-center gap-1 text-[#17324D] font-medium">
                        <Waves className="w-3.5 h-3.5 text-[#1268B3]" />
                        {item.spill_area_km2 ? `${item.spill_area_km2} km²` : "Area N/A"}
                      </span>
                      <span>•</span>
                      <span className="flex items-center gap-1 text-[#A86A00] font-medium">
                    ...
                    <div className="flex items-center gap-2">
                      <Link
                        ...
                        className="text-[11px] text-[#1268B3] hover:text-[#0B3A66] flex items-center gap-0.5 font-bold"
                      >
                        <span>Details</span>
        ...
        <div className="lg:col-span-5">
          {selectedItem ? (
            <div className="bg-white p-5 rounded-xl border border-[#D9E8F2] shadow-sm space-y-5 sticky top-6">
              {/* Header */}
              <div className="flex items-start justify-between pb-4 border-b border-[#EAF3F8]">
                <div>
                  ...
                  <p className="text-xs text-[#5E7183] mt-1">
                    ...
                    <span className="font-bold text-[#17324D]">{selectedItem.urgency_level} PRIORITY</span>
                  </p>
                </div>

                <div className="text-right">
                  <div className="text-2xl font-extrabold text-[#0B3A66] font-mono">
                    {selectedItem.priority_score.toFixed(1)}
                  </div>
                  <div className="text-[10px] text-[#8A9AA8] uppercase tracking-wider font-semibold">Priority Index</div>
                </div>
              </div>
              ...
              <div className="p-3.5 rounded-xl bg-[#F3FAFE] border border-[#D9E8F2] space-y-2">
                <span className="text-[10px] uppercase font-bold tracking-wider text-[#1268B3] flex items-center gap-1.5">
                  <Shield className="w-3.5 h-3.5 text-[#1268B3]" />
                  Why is this incident ranked #{selectedItem.rank}?
                </span>
                <p className="text-xs text-[#17324D] leading-relaxed">
                  {selectedItem.reason}
                </p>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between text-xs">
                  <span className="font-bold text-[#17324D] uppercase tracking-wider text-[11px] flex items-center gap-1.5">
                    <SlidersHorizontal className="w-3.5 h-3.5 text-[#1268B3]" />
                    Priority Factor Breakdown
                  </span>
                  <span className="text-[10px] text-[#8A9AA8] font-medium">Sum = 100.0 Max</span>
                </div>
                ...
                <div className="space-y-2.5">
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-xs">
                      <span className="text-[#5E7183]">Risk Severity Component</span>
                      <span className="font-mono text-[#17324D] font-bold">
                        {selectedItem.factors.risk_score_component.toFixed(1)} / 35.0 pts
                      </span>
                    </div>
                    <div className="w-full bg-[#EAF3F8] rounded-full h-1.5 overflow-hidden">
                      <div
                        className="bg-[#C6283D] h-full rounded-full"
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-xs">
                      <span className="text-[#5E7183]">Shoreline Impact Urgency &amp; ETA</span>
                      <span className="font-mono text-[#17324D] font-bold">
                    ...
                    <div className="w-full bg-[#EAF3F8] rounded-full h-1.5 overflow-hidden">
                      <div
                        className="bg-[#A86A00] h-full rounded-full"
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-xs">
                      <span className="text-[#5E7183]">Ecological / Marine Habitat Proximity</span>
                      <span className="font-mono text-[#17324D] font-bold">
                    ...
                    <div className="w-full bg-[#EAF3F8] rounded-full h-1.5 overflow-hidden">
                      <div
                        className="bg-[#087F68] h-full rounded-full"
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-xs">
                      <span className="text-[#5E7183]">Spill Surface Area &amp; Volume</span>
                      <span className="font-mono text-[#17324D] font-bold">
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-xs">
                      <span className="text-[#5E7183]">Operational Uncontained Status</span>
                      <span className="font-mono text-[#17324D] font-bold">
                    ...
                    <div className="w-full bg-[#EAF3F8] rounded-full h-1.5 overflow-hidden">
                      <div
                        className="bg-[#168DCC] h-full rounded-full"
              ...
              <div className="grid grid-cols-2 gap-2 text-xs pt-1">
                <div className="p-2.5 bg-[#F8FCFF] rounded-lg border border-[#D9E8F2]">
                  <div className="text-[10px] text-[#5E7183]">Shoreline Arrival ETA</div>
                  <div className="font-bold text-[#A86A00] font-mono mt-0.5">
                ...
                <div className="p-2.5 bg-[#F8FCFF] rounded-lg border border-[#D9E8F2]">
                  <div className="text-[10px] text-[#5E7183]">Distance to Coast</div>
                  <div className="font-bold text-[#1268B3] font-mono mt-0.5">
              ...
              <div className="text-[10px] text-[#8A9AA8] pt-2 border-t border-[#EAF3F8] flex items-center justify-between">
                <span>Deterministic Operational Dispatch Algorithm</span>
                <span>Auto-refreshes on risk update</span>
              </div>
            </div>
          ) : (
            <div className="bg-white p-12 text-center text-[#5E7183] text-xs rounded-xl border border-[#D9E8F2]">
